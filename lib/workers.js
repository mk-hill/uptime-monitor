...
// Only see detailed logs if index.js is started with NODE_DEBUG=workers
...
const workers = {
  // Init script
  init() {
    // Send to console in cyan
    console.log('\x1b[36m%s\x1b[0m', 'Background workers running');
        ...
        checks.forEach(check => {
          ...
          dataLib.read('checks', check, (err, originalCheckData) => {
            if (!err && originalCheckData) {
              ...
              debug('Error reading a check\'s data');
            }
          });
        });
      } else {
  ...
  processCheckOutcome(originalCheckData, checkOutcome) {
    ...
    dataLib.update('checks', newCheckData.id, newCheckData, err => {
      if (!err) {
        ...
        debug(`Error saving updates to check ${newCheckData.id}`);
      }
    });
  },
  ...
  alertUserToStatusChange(newCheckData) {
    ...
    helpers.sendTwilioSms(newCheckData.userPhone, msg, err => {
      if (!err) {
        debug('Success: User alerted to status change via sms', msg);
      } else {
        debug(
