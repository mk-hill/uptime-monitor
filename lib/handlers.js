...
// Performance logging with NODE_DEBUG=performance
...
const handlers = {
  ...
  notFound(data, callback) {
    ...
    helpers.getTemplate('404', templateData, (err, str) => {
      if (!err && str) {
  ...
  accountVerify(data, callback) {
    // Reject any request that isn't a GET
    if (data.method === 'get') {
      // Prepare data for interpolation
      const templateData = {
        'head.title': 'Verify Phone',
        'head.description': 'Verify your phone number',
        'body.class': 'accountVerify',
      };

      // Read in html template as a string
      helpers.getTemplate('accountVerify', templateData, (err, str) => {
        if (!err && str) {
          // Add universal header and footer
          helpers.addUniversalTemplates(
            str,
  ...
  _tokens: {
    ...
    // Optional data: none
    post(data, callback) {
      // _performance.mark('entered tokens - post');
      ...
      if (phone && password) {
        // _performance.mark('start user lookup');
        ...
        dataLib.read('users', phone, (err, userData) => {
          ...
          if (!err && userData) {
            ...
            if (hashedPassword === userData.hashedPassword) {
              ...
              // _performance.mark('token creation complete');

              // Store the token
              // _performance.mark('start storing token');
            ...
            dataLib.read('users', userPhone, (err, userData) => {
              if (!err && userData) {
                ...
                if (userChecks.length < config.maxChecks) {
                  // Verify that the provided URL has DNS entries and can resolve
                  ...
                  // Only continue and create check if DNS resolves, prevent system resources
                  // from being wasted on repeating useless checks
                    ...
                    if (!err && records) {
                      ...
                      dataLib.create('checks', checkId, checkObj, err => {
                        if (!err) {
                          ...
                          callback(500, {
                            error: 'Unable to create new check',
                          });
                        }
                      });
                    } else {
                      callback(400, {
                        error:
                          'The hostname of the specified URL did not resolve to any DNS entries',
                      });
                    }
                  });
                } else {
